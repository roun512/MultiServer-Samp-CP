...
class codadmin
{
	 		...
	 		if($this->app->info->IsPlayerBanned($name) == false) {
	 			...
	 			if($info->adminlevel > $this->app->user->admin) {
	 				...
	 				try {
	 					...
	 					$st = $this->app->coddb->prepare("INSERT INTO banned (name, ip, reason, admin, unban) VALUES (?, ?, ?, ?, ?)");
	 					$st->bindParam(1, $name, PDO::PARAM_STR);
	...
	public function GetGameAdminLogs($search = null, $offset)
	{
	 	try {
	 		
	 		$query = "SELECT * FROM cmd_log";
	 		if($search != null)
	 		...
	 		if($search != null)
	 			$st->bindParam(':command', $search, PDO::PARAM_STR);

 			$st->bindParam(':limit', $this->app->config['logs_limit'], PDO::PARAM_INT);
	...
	public function GetGameAdminLogsCount($search = null) {
		try {
			$query = "SELECT COUNT(*) as total FROM cmd_log";
			if($search != null)
	 			$query .= " WHERE command = :command";
			$st = $this->app->coddb->prepare($query);
			if($search != null)
